...
let parseCode (a:string):uint16 =
    ...
    with
    | _ ->
        printfn "Can't parse line as hackVM instruction. Expected string representation of 16 bit binary value like '1110000011000111'. Got:"
        printfn "%A" a
        exit 0
...
[<EntryPoint>]
let main argv =
    let vmParams = parseParams (argv |> List.ofArray) {debug=false;tick=10;fileName=None}
    let rom = 
