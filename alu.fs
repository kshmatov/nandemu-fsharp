...
let isMask what mask = what &&& mask = mask
    ...
    if op &&& firstBit = 0us then
        ...
        ComputationOp
            { jump = jumpMask &&& op |> int32 |> enum<JumpCode>
              ...
              src =
                if op &&& srcMask = 0us then
                    OperandSource.ARegister
                else
...
let makeCOp (op: OpDesc) (regs: alu) (mem: State) : alu =
    if regs.Debug then
        printfn
            $"pc: {regs.ProgramCounter}; d: {regs.DRegister}; a: {regs.ARegister}; mem[a]:{mem.GetMem(regs.ARegister |> uint16)}"
    ...
    if op.dest.toMem then
        mem.SetMem (regs.ARegister |> uint16) res
...
let oneStep (mem: State) (regs: alu) : alu option =
    ...
    match op with
    | _ when regs.ProgramCounter > 0x7fffus -> None // Address is coded in 15bits
...
let RunAlu (rom: memory.Instruction array) = RunAluSettings rom 1 false
